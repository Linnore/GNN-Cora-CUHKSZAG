...
class GraphSage(pl.LightningModule):
    """This class define a GraphSAGE model for linkage prediction (recommendation for nodes). 
    """
    def __init__(self, dataset, input_dim, hparams, writer_acc=None, writer_loss=None):
        """This class define a GraphSAGE model for linkage prediction (recommendation for nodes). 
        IMPORTANT: After create an object of this class, please do manually call your object.data_processing() to prepare the model on your device.
        ...
        elif self.aggregator_type == "softmax":
            self.aggr = aggr.SoftmaxAggregation(learn=True)
        elif self.aggregator_type == "power":
            self.aggr = aggr.PowerMeanAggregation(learn=True)
        elif self.aggregator_type == "sort":
    ...
    def forward(self, x, edge_index):
        """The forward path of this model: node features -> GraphSAGE layers -> node embedding.
    ...
    def data_processing_transductive(self, seed=118010142):
        ...
        # data.edge_train_mask = torch.ones(data.num_edges, dtype=bool)
        # data.edge_val_mask = torch.ones(data.num_edges, dtype=bool)
        # data.edge_test_mask = torch.ones(data.num_edges, dtype=bool)
        
        data = data.to(self.device)

        # data.edge_train_mask = torch.logical_and(
        #     data.train_mask[data.edge_index[0]], data.train_mask[data.edge_index[1]])

        # data.edge_val_mask = torch.logical_or(
        #     data.val_mask[data.edge_index[0]], data.val_mask[data.edge_index[1]])
        # data.edge_test_mask = torch.logical_or(
        #     data.test_mask[data.edge_index[0]], data.test_mask[data.edge_index[1]])
        ...
        # neg_data.edge_train_mask = torch.logical_and(
        #     neg_data.train_mask[neg_data.edge_index[0]], neg_data.train_mask[neg_data.edge_index[1]])

        # neg_data.edge_val_mask = torch.logical_or(
        #     neg_data.val_mask[neg_data.edge_index[0]], neg_data.val_mask[neg_data.edge_index[1]])
        # neg_data.edge_test_mask = torch.logical_or(
        #     neg_data.test_mask[neg_data.edge_index[0]], neg_data.test_mask[neg_data.edge_index[1]])
    ...
    def data_processing_inductive(self):
        """ This is the default data processsing method for inductive learning!
        This method prepare the data loaders given the data with proper train-val-test split through properties: train_mask, val_mask, and test_mask.
        Make sure the GCN object received its dataset parameter as a dataset object of torch_geometric.data.Dataset.
        After calling this method, the GCN object would have all the following graphs stored in self.pos_data and self.neg_data:
            the whole positive graph: self.pos_data["all"]
        ...
        Training graph: edges with both ends in {node_training}
        Validation graph: edges with at least one end in {node_val}
        Testing graph: edges with at least one end in {node_test}
        """
        data.edge_train_mask = torch.logical_and(
            data.train_mask[data.edge_index[0]], data.train_mask[data.edge_index[1]])
        ...
        """Process the train-val-test split of the negative graph:
